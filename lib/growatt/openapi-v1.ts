...
export function mapV1PlantLatest(plantData: unknown, powerData: unknown, context: { plantId: string; device: GrowattDeviceSummary }, nowLocal: string, timezone = "Europe/Budapest"): GrowattLatestEnergy {
  ...
  return { plantId: context.plantId, deviceId: context.device.id, deviceSerialNumber: context.device.serialNumber, deviceType: context.device.type, deviceModel: context.device.model, deviceStatus: context.device.status, measuredAt, currentPowerW: latestPower?.power ?? null, ...energy, gridImportPowerW: null, gridExportPowerW: null, loadPowerW: null, batteryChargePowerW: null, batteryDischargePowerW: null, batterySocPercent: null, source: "growatt", rawCapabilities: capabilities };
}
...
export function mapV1DeviceEnergy(data: unknown, context: { plantId: string; device: GrowattDeviceSummary }): GrowattLatestEnergy {
  ...
  return { plantId: context.plantId, deviceId: context.device.id, deviceSerialNumber: context.device.serialNumber, deviceType: context.device.type, deviceModel: context.device.model, deviceStatus: context.device.status, measuredAt: measured && Number.isFinite(Date.parse(measured)) ? new Date(measured).toISOString() : null, currentPowerW: fields.currentPowerW, todayEnergyKwh: fields.todayEnergyKwh, monthEnergyKwh: null, yearEnergyKwh: null, lifetimeEnergyKwh: fields.lifetimeEnergyKwh, gridImportPowerW: fields.gridImportPowerW, gridExportPowerW: fields.gridExportPowerW, loadPowerW: fields.loadPowerW, batteryChargePowerW: fields.batteryChargePowerW, batteryDischargePowerW: fields.batteryDischargePowerW, batterySocPercent: fields.batterySocPercent, source: "growatt", rawCapabilities: Object.entries(fields).filter(([, value]) => value !== null).map(([key]) => key) };
}
