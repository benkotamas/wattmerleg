...
async function retryRateLimitOnce<T>(operation: () => Promise<T>, sleep: (milliseconds: number) => Promise<void>, retryDelayMs: () => number): Promise<T> { try { return await operation(); } catch (error) { if (asGrowattError(error).code !== "GROWATT_RATE_LIMITED") throw error; await sleep(retryDelayMs()); return operation(); } }

export async function syncGrowattHistory(options: { userId: string; startDate: string; endDate: string; database: HistoricalDatabase; api?: HistoricalApi; now?: Date; sleep?: (milliseconds: number) => Promise<void>; retryDelayMs?: () => number }): Promise<GrowattSyncResult> {
  ...
  const plant = single(mapV1Plants(await retryRateLimitOnce(() => api.plantList(), sleep, retryDelayMs))), timezone = validGrowattTimezone(plant.timezone), currentLocalDate = localIsoDate(now, timezone);
  ...
  for (const chunk of chunks) {
    try {
      const raw = await retryRateLimitOnce(() => api.plantEnergyHistory(plant.id, chunk.startDate, chunk.endDate, "day", 1, 20), sleep, retryDelayMs), fetchedAt = new Date().toISOString();
