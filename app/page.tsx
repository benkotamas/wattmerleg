...
import { CurrentFinancialPositionCard } from "@/components/energy/current-financial-position-card";
...
export default function DashboardPage() {
  ...
  const cards = [
    { label: "Hálózatból vételezett – kézi mérő", value: formatKwh(summary.consumption), note: formatComparison(comparison?.consumptionPercent), icon: ArrowDownToLine, color: "text-orange-600 bg-orange-50" },
    { label: "Hálózatba betáplált – kézi mérő", value: formatKwh(summary.production), note: formatComparison(comparison?.productionPercent), icon: ArrowUpFromLine, color: "text-emerald-700 bg-emerald-50" },
    { label: "Nettó hálózati egyenleg – kézi mérő", value: formatKwh(summary.balance), note: formatComparison(comparison?.balancePercent), icon: Scale, color: "text-blue-700 bg-blue-50" },
    { label: "Becsült összeg – kézi mérő", value: formatHuf(summary.estimatedAmount), note: summary.estimatedAmount >= 0 ? "Eddigi várható fizetendő" : "Eddigi várható jóváírás", icon: CircleDollarSign, color: "text-violet-700 bg-violet-50" },
  ];
  return (
    <AppShell>
      ...
      <CurrentFinancialPositionCard/>
      <div className="mb-3 flex flex-wrap items-end justify-between gap-2"><div><p className="text-xs font-black uppercase tracking-wider text-slate-500">Kézi mérőállás alapján</p><h2 className="text-lg font-black">Kézi ellenőrző mérleg</h2></div><p className="text-xs text-slate-500">Legutóbbi kézi mérés: {formatDate(forecast.referenceDate)}</p></div>
