...
export async function POST(request: NextRequest) {
  ...
  try { readings = await readAllMeterReadings((from, to) => auth.client.from("meter_readings").select("id,user_id,reading_at,consumption_meter_kwh,production_meter_kwh,note,settlement_period_id,created_at,updated_at").eq("user_id", auth.userId).order("reading_at", { ascending: true }).order("id", { ascending: true }).range(from, to)); }
  catch { return fail(503, "SNAPSHOT_INPUT_READ_FAILED"); }
  const [pvResult, snapshotResult] = await Promise.all([
  ...
  if (pvResult.error || snapshotResult.error) return fail(503, "SNAPSHOT_INPUT_READ_FAILED");
