...
export async function POST(request: NextRequest) {
  ...
  try { const result = await withGrowattSyncLock(context.userId, () => syncGrowattHistory({ userId: context.userId, startDate: body.startDate as string, endDate: body.endDate as string, database: context.client as unknown as HistoricalDatabase })); return NextResponse.json(result, { status: result.partial ? 207 : 200, headers: { "Cache-Control": "no-store" } }); }
