...
export default function SettingsPage() {
  ...
  const { period, periods, readings, allReadings, billingSnapshots, billingSnapshotsAvailable, tariff, tariffFromDatabase, loading, error, refresh } = useEnergyData();
  ...
  async function closePeriod() {
    ...
    if (error) { setClosing(false); return setMessage(error.message); }
    let snapshotSaved = false;
    if (billingSnapshotsAvailable) {
      const response = await fetch(`/api/billing/snapshots/${period.id}`, { method: "POST", credentials: "same-origin", cache: "no-store" });
      snapshotSaved = response.ok;
    }
    setClosing(false);
    setMessage(snapshotSaved ? "Az időszak lezárult, a díjszámítási pillanatkép elkészült, az új időszak elindult." : "Az időszak lezárult és az új időszak elindult. A pillanatképhez futtasd a 016-os migrációt, majd rögzítsd a lezárt időszaknál.");
    void refresh();
  }
  ...
  return (
    <AppShell>
      ...
      <PeriodList periods={periods} readings={allReadings} tariff={tariff} billingSnapshots={billingSnapshots} billingSnapshotsAvailable={billingSnapshotsAvailable} onBillingChanged={refresh}/>
