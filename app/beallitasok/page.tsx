...
export default function SettingsPage() {
  ...
  async function saveTariff() {
    ...
    if (values.some(value => !Number.isFinite(value) || value < 0) || form.annual_closing_month < 1 || form.annual_closing_month > 12 || form.annual_closing_day < 1 || form.annual_closing_day > 31) return setMessage("Ellenőrizd a megadott pozitív értékeket és a zárási dátumot.");
    setSaving(true);
  ...
  return (
    <AppShell>
      ...
      <section className="card mt-3 p-5"><h2 className="text-lg font-black">Éves elszámolás</h2><p className="mt-2 text-sm text-slate-600">Következő tervezett zárás: <b>{period ? formatDate(closingDateForPeriod(period.start_date, tariff)) : "–"}</b>. A zárás nem történik meg automatikusan.</p>{summary && <div className="mt-4 grid grid-cols-2 gap-2 rounded-xl bg-slate-50 p-3 text-sm"><span>Fogyasztás</span><b className="text-right">{formatKwh(summary.consumption)}</b><span>Termelés</span><b className="text-right">{formatKwh(summary.production)}</b><span>Becsült összeg</span><b className="text-right">{formatHuf(summary.estimatedAmount)}</b></div>}<button disabled={closing || !period} onClick={() => void closePeriod()} className="mt-4 w-full rounded-xl bg-slate-900 p-3 font-bold text-white">{closing ? "Lezárás…" : "Éves időszak lezárása"}</button></section>
