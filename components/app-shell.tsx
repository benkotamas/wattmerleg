...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="mx-auto min-h-screen max-w-[1440px] pb-24 md:pb-10">
      <header className="flex items-center justify-between px-4 py-4 sm:px-6 md:px-8 md:py-5">
        ...
        <nav className="hidden gap-1 md:flex">
          {links.map(({ href, label }) => (
            <Link key={href} href={href} aria-current={pathname===href||href!=="/"&&pathname.startsWith(`${href}/`)?"page":undefined} className={`rounded-xl px-3 py-2 text-sm font-semibold ${pathname===href||href!=="/"&&pathname.startsWith(`${href}/`) ? "bg-emerald-100 text-emerald-800" : "text-slate-600 hover:bg-white"}`}>{label}</Link>
          ))}
        </nav>
      </header>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-20 mx-auto flex max-w-2xl justify-around border-t border-slate-200 bg-white/95 px-1 pb-[max(.45rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur md:hidden">
        {links.map(({ href, label, icon: Icon }) => (
          <Link key={href} href={href} aria-current={pathname===href||href!=="/"&&pathname.startsWith(`${href}/`)?"page":undefined} className={`flex min-w-0 flex-1 flex-col items-center gap-1 text-[10px] font-bold ${pathname===href||href!=="/"&&pathname.startsWith(`${href}/`) ? "text-emerald-700" : "text-slate-500"}`}>
