...
import { fetchGrowattUiData, formatGrowattEnergy, formatGrowattMeasuredAt, formatGrowattPower, freshnessLabel, growattCapabilityLabels, growattErrorMessage, growattFreshness } from "@/lib/growatt/ui";
...
export function SolarProductionCardView({ diagnostic, data, error, loading, onRefresh }: { diagnostic: boolean; data: GrowattUiData | null; error: GrowattUiError | null; loading: boolean; onRefresh: () => void }) {
  ...
  return <section className="card mt-4 min-w-0 overflow-hidden p-5" aria-labelledby={diagnostic ? "growatt-settings-title" : "growatt-overview-title"}>
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div className="min-w-0"><p className="text-xs font-bold uppercase tracking-wide text-emerald-700">Read-only Growatt kapcsolat</p><h2 id={diagnostic ? "growatt-settings-title" : "growatt-overview-title"} className="mt-1 text-lg font-black">{diagnostic ? "Growatt napelem-integráció" : "Napelem – inverteradatok"}</h2></div>
...
function Production({ latest, freshness }: { latest: NonNullable<GrowattUiData["latest"]>; freshness: ReturnType<typeof growattFreshness> }) {
  ...
  return <div className="mt-5">
    ...
    <div className="mt-4 rounded-xl border border-blue-100 bg-blue-50 p-3 text-sm text-blue-950"><b>Mit jelent ez?</b><p className="mt-1">Az inverteres termelés nem azonos a hálózatba visszatáplált energiával. A különbség egy része helyben felhasznált napenergia lehet.</p></div>
  </div>;
}
...
function Diagnostic({ latest, connected, capabilities }: { latest: NonNullable<GrowattUiData["latest"]>; connected: boolean; capabilities: string[] }) {
  return <div className="mt-4 space-y-3 text-sm"><dl className="grid gap-2 sm:grid-cols-2"><Row label="Konfiguráció" value="Konfigurálva"/><Row label="Kapcsolat" value={connected ? "Kapcsolódva" : "Hiba"}/>{latest.deviceType && <Row label="Inverter típusa" value={latest.deviceType}/>} {latest.deviceModel && <Row label="Modell" value={latest.deviceModel}/>} {latest.deviceStatus && <Row label="Eszköz állapota" value={latest.deviceStatus}/>}<Row label="Utolsó sikeres mérési idő" value={formatGrowattMeasuredAt(latest.measuredAt)}/></dl><div className="rounded-xl bg-slate-50 p-3"><p className="font-bold">Elérhető adatok</p>{capabilities.length ? <ul className="mt-2 flex flex-wrap gap-2">{capabilities.map(item => <li key={item} className="rounded-full bg-white px-2 py-1 text-xs">{item}</li>)}</ul> : <p className="mt-1 text-slate-600">Nincs megjeleníthető capability.</p>}</div><p className="text-xs text-slate-500">Az API-hozzáférés szerveroldali beállítás. Titkos vagy egyedi azonosító ezen a felületen nem jelenik meg.</p></div>;
}
function Row({ label, value }: { label: string; value: string }) { return <div className="flex min-w-0 justify-between gap-3 rounded-xl bg-slate-50 p-3"><dt className="text-slate-600">{label}</dt><dd className="break-words text-right font-bold">{value}</dd></div>; }
