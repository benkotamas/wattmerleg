...
describe("SolarProductionCard", () => {
  ...
  it("a 429 átmeneti állapot, megtartja az előző adatot és nem piros alert", () => { const error = { status: 429, code: "GROWATT_RATE_LIMITED", message: "raw technical detail" } satisfies GrowattUiError; const html = render({ data, error }); expect(html).toContain('role="status"'); expect(html).toContain("Átmeneti Growatt-korlátozás"); expect(html).toContain("2,84 kW"); expect(html).not.toContain('role="alert"'); expect(html).not.toContain("raw technical detail"); });
  it("aktív cooldownnál kiírja a következő próbálkozást és tiltja a Frissítés gombot", () => { const retryAt = Date.now() + 15 * 60_000; const html = render({ data: { ...data, rateLimitedUntil: retryAt } }); expect(html).toContain("Következő próbálkozás:"); expect(html).toMatch(/<button[^>]*disabled/); });
  it("snapshot nélküli 429 nem állítja, hogy korábbi adat látható", () => { const error={status:429,code:"GROWATT_RATE_LIMITED",message:"hidden",retryAt:Date.now()+900000};const html=render({data:null,error});expect(html).toContain("Jelenleg nincs megjeleníthető korábbi adat");expect(html).not.toContain("A korábbi adat továbbra is látható");expect(html).toMatch(/<button[^>]*disabled/); });
  it("a nem átmeneti hiba továbbra is alert", () => { const error = { status: 503, code: "GROWATT_TIMEOUT", message: "hidden" } satisfies GrowattUiError; expect(render({ data: null, error })).toContain('role="alert"'); });
